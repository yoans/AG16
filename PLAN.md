...
- [x] Fix iPhone 16 Pro (402×874) layout — header overflow, cramped controls ✅
  - [x] Collapse header into 2 clear rows: [logo + play + undo/redo] [presets + actions] ✅
  - [x] Hide text labels on all header buttons (icon-only on mobile) ✅
  - [x] Undo/redo group: icon-only, no text labels on mobile ✅
  - [x] Randomize/Clear buttons: icon-only on mobile, move into actions row ✅
  - [x] Ensure safe-area-inset for notch/dynamic island ✅
  - [x] Canvas should fill available width with no horizontal scroll ✅
  - [x] Side panels: fluid grid, no overflow, stacked vertically ✅
  - [x] Footer selects: full-width stacked on narrow screens ✅
...
- [x] Fix iPad (820×1180 portrait, 1180×820 landscape) layout ✅
  ...
  - [x] Landscape: use desktop 3-column layout (panel–canvas–panel) ✅
  - [ ] Test both orientations with rotation (requires real device)
- [x] Add `env(safe-area-inset-*)` padding for iOS notch/home indicator ✅
- [x] Test slider overlays on mobile — fullscreen touch interaction ✅
- [x] Test channel select overlay on mobile — grid layout, touch targets ✅
...
- [x] Verify analytics working in production (implementation verified, check GA4 dashboard) ✅
...
- [x] Copyright footer
- [x] Touch targets: all primary buttons ≥44×44px on mobile
- [x] Canvas fills width, no horizontal scroll (verified iPhone 16 Pro)
- [x] iPad landscape: desktop 3-column layout (verified 1180×820)
- [x] Slider overlays: fullscreen touch-friendly on mobile
- [x] Channel select overlay: 4-column grid, 48px items on mobile
- [x] Footer selects: full-width stacked, 44px height on mobile
- [x] Master volume button touch target fix (removed inline minHeight override)
